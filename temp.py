...
================================================================================
 ...
 Structure:
   ...
   9. VALIDATION                - gate 1 (v1 anchor), gate 2 (B=0 linearity),
                                  ...
                                  gate 6 (both sides vs a brute force)
  10. PLOTS
  11. RUN
...
class TraderSpec:
    ...
    q_floor: float = 0.0          
    side: str = "B"               # "A": start pounds, target dollars (T1 family)
                                  # "B": start dollars, target pounds (T4 family)


# ------------------------------------------------------------------------------
# The side's RULES live in fx_mechanics: what the MM takes (mm_accepts), what an
# accepted trade pays (trade_rate), and what the BdC pays (bdc_payoff_per_unit)
# are all game mechanics, stated in prices, and all take the side. Exactly one
# side-aware thing is a SOLVER detail, and it is this:
#
...
# E[a5 | a4] = a4 exactly and no quadrature is needed. See terminal_matrix.
# ------------------------------------------------------------------------------
...
def solve_round(spec, grids, Wnext, a):
    ...
    for t in range(nq):
        ...
        rate = bdc_payoff_per_unit(X, f, spec.side) # BdC pays (1-fee)*X per GBP
...
def print_game_trace(trace):
    ...
    plays' table."""
    spec = trace["spec"]
    INIT, TARG = currency_symbols(spec.side)
    print(f"{'':2}anchor   hidden X      offer            verdict     "
          f"-> {INIT} left   {TARG} banked")
    for r in trace["rounds"]:
        ...
        for o in r["offers"]:
            ...
            print(f"  offer {o['offer_no']}: {o['P']:.4f} "
                  ...
                  f"z={o['z']:+.2f})  "
                  f"sell {o['q']:>8,.0f}   {verdict:>6}   "
                  f"-> c={o['c']:>8,.0f}   d={o['d']:>10,.0f}")
        if r["bdc_dump"] > 1e-6:
            print(f"  BdC: dump {INIT} {r['bdc_dump']:>8,.0f} at the BdC rate"
    ...
    print(f"Settlement a5={trace['a5']:.4f} | end c={INIT} {trace['c']:,.0f}, "
          f"d={TARG} {trace['d']:,.0f} (target {TARG} {spec.T:,.0f}, "
          f"{'short %s %.0f' % (TARG, short) if short > 1 else 'target met'}) | "
          f"W={INIT} {trace['W']:,.0f}  P/L={trace['pl']*100:+.2f}%")
...
def plot_game(trace, save_path=None, ax_rate=None, ax_inv=None, title=None):
    ...
    spec = trace["spec"]
    SIDE = spec.side
    INIT, TARG = currency_symbols(SIDE)
    ...
    for r in trace["rounds"]:
        ...
        if r["bdc_dump"] > 1e-6:
            ax_rate.scatter([x + 0.4],
                            [bdc_payoff_per_unit(r["X"],
                                                 spec.params.bdc_fee, SIDE)],
    ...
    ax_inv.step(cser_x, np.array(cser_y) / spec.L, where="post",
                color="tab:blue", lw=1.8, label=f"{INIT} left / L")
    ax_inv.step(dser_x, np.array(dser_y) / spec.T, where="post",
                color="tab:green", lw=1.8, label=f"{TARG} banked / T")
...
def gate1_v1_anchor(K=3, print_progress=True, side="A"):
    ...
    Run BOTH sides. The seller's anchor is a closed form; the buyer's is one
    quadrature (E[1/X ; X > P] has no elementary antiderivative), but it is
    just as exact and just as independent of this file.

    Note what this gate does NOT reach: with terminal_mode="dollars" the
    terminal layer is the identity in d and carries no anchor dependence, so
    there is no settlement conversion and no anchor interpolation here. Gate 6
    ...
    spec = TraderSpec(rounds=1, K=K, B=0.0, all_or_nothing=True,
                      terminal_mode="dollars", side=side)
    ...
    v1 = solve_v1(max_offers=K, side=side)
    worst = 0.0
...
def gate3_and_4(sol, n_paths=20_000, seed=1, print_progress=True):
    ...
    if print_progress:
        ...
        _i, _t = currency_symbols(sol.spec.side)
        print(f"  money conservation: worst {_i} error "
              f"{res['pound_err']:.2e}, worst {_t} error "
              f"{res['dollar_err']:.2e}")
...
def gate5_brackets(sol, res, print_progress=True):
    ...
    if print_progress:
        _i, _t = currency_symbols(spec.side)
        print(f"  always-BdC floor    {_i} {W_bdc:,.1f}  ({pl_bdc*100:+.3f}%)")
        print(f"  DP                  {_i} {sol.value:,.1f}  ({sol.pl()*100:+.3f}%)")
        print(f"  clairvoyant ceiling {_i} {W_cv:,.1f}  ({(W_cv-spec.L)/spec.L*100:+.3f}%)")
...
def gate6_brute_force(side="A", print_progress=True):
    """One round, one all-or-nothing offer, against a DIRECT enumeration.

    The round is rebuilt from scratch in real coordinates: prices read straight
    off the grid, acceptance checked against the raw rule, the BdC optimised by
    hand, and settlement done with its own Gauss-Hermite quadrature rather than
    the solver's adaptive kappa. None of the side machinery (_rate_at,
    trade_rate) is shared -- comparing against code that used it would only
    prove it agrees with itself.

    RUN BOTH SIDES. Side A is the side that can be anchored independently (gate
    1 ties it to v1 and v0), so passing on A is what makes this enumeration
    trustworthy in the first place; passing on B then means the reflection is
    right. A B-only version would be unvalidated code checking unvalidated code.

    The anchor grid is deliberately fine here. The buyer's terminal is LINEAR in
    the anchor, so interpolating it across anchor nodes is exact; the seller's
    carries kappa(a4), which is convex, so its interpolation converges only as
    the anchor grid refines. A coarse anchor grid would swamp the thing this
    gate is meant to test with an error that has nothing to do with the side.
    """
    gh_x, gh_w = np.polynomial.hermite_e.hermegauss(64)
    gh_w = gh_w / gh_w.sum()
    worst = 0.0
    for L, T, A, B in ((100_000., 125_000., 0.02, 0.03),
                       (500_000., 396_000., 0.00, 0.20),
                       (300_000., 260_000., 0.03, 0.05)):
        spec = TraderSpec(rounds=1, K=1, all_or_nothing=True, side=side,
                          L=L, T=T, A=A, B=B)
        grids = Grids(spec, n_offer=41, quad_per_cell=7, n_c=3, n_d=161,
                      n_a=65, a_halfwidth_sds=5.0)
        Wterm = np.stack([terminal_matrix(spec, grids, a4) for a4 in grids.a])
        entry, _, _ = solve_round(spec, grids, Wterm, spec.params.a0)
        solver = float(entry[grids.n_c - 1, 0])

        p = spec.params
        a0, sd, f = p.a0, p.sd, p.bdc_fee

        def settle(u, v, X):
            # seller converts the target currency at 1/a5 -> needs E[1/a5 | X];
            # buyer converts at a5, and the martingale gives E[a5 | X] = X
            conv = (X if side == "B"
                    else float(np.sum(gh_w / np.maximum(X + sd * gh_x, 1e-9))))
            return u * (1 - A) + (v - B * max(T - v, 0.0)) * conv

        def refused(X):                 # the initial currency to the BdC, or held
            bdc = (1 - f) / X if side == "B" else (1 - f) * X
            return max(settle(L - L * fr, L * fr * bdc, X)
                       for fr in grids.f_all)

        best = -np.inf
        for z in grids.z:
            P = a0 - sd * z if side == "B" else a0 + sd * z
            if P <= 0:
                continue
            gained = L / P if side == "B" else L * P
            # the buyer's bid is taken iff X < P, the seller's ask iff X > P
            lt, ht, lr, hr = ((a0 - 10 * sd, P, P, a0 + 10 * sd) if side == "B"
                              else (P, a0 + 10 * sd, a0 - 10 * sd, P))
            acc, _ = quad(lambda X: settle(0.0, gained, X) * norm.pdf(X, a0, sd),
                          lt, ht, limit=200, epsabs=1e-10, epsrel=1e-10)
            rej, _ = quad(lambda X: refused(X) * norm.pdf(X, a0, sd),
                          lr, hr, limit=200, epsabs=1e-10, epsrel=1e-10)
            best = max(best, acc + rej)
        rel = abs(solver - best) / L
        worst = max(worst, rel)
        if print_progress:
            print(f"  side {side}  L={L:>9,.0f}  solver {solver:>12,.3f}  "
                  f"brute force {best:>12,.3f}  rel gap {rel:.2e}")
    assert worst < 5e-5, f"gate 6 FAILED on side {side}: off by {worst:.2e}"
    return worst
...
def plot_kink_and_policy(sol, save_path="v2_fig1_policy.png"):
    ...
    ax1.set_xlabel(f"{TARG} banked, d ({TARG}k)")
    ax1.set_ylabel(f"marginal value of one more {TARG} ({INIT} per {TARG})")
    ax1.set_title(f"Marginal value of a unit of {TARG}:\n"
                  f"worth (1+B) x the settlement rate while short of T, "
                  f"just the rate once past it")
...
def plot_value_surface(sol, save_path="v2_fig2_value.png"):
    ...
    ax.annotate("the actual start (L, 0)", (2, spec.L / 1000 * 0.96),
                fontsize=9)
    ax.set_xlabel(f"{TARG} banked, d ({TARG}k)")
    ax.set_ylabel(f"{INIT} held, c ({INIT}k)")
...
if __name__ == "__main__":
    ...
    gate1_v1_anchor(K=3, side="A")
    gate1_v1_anchor(K=3, side="B")
    print("  gate 1 PASSED: v2 collapses to v1 (and v0 through it), both sides")
 
    print("\n[3] Headline solve")
    spec = TraderSpec()
    _i, _t = currency_symbols(spec.side)
    print(f"  card: L={_i} {spec.L:,.0f}, T={_t} {spec.T:,.0f}, A={spec.A:.0%}, "
          f"B={spec.B:.0%}, {spec.rounds} rounds, K={spec.K}")
    t0 = time.time()
    sol = solve_v2(spec, print_progress=True)
    print(f"  solved in {time.time() - t0:.0f}s")
    print(f"  E[final wealth] = {_i} {sol.value:,.1f}   "
    ...
    # ---- the same machinery, for a trader starting in the other currency ----
    print("\n" + "=" * 74)
    print("[8] THE BUYER'S SIDE (card T4: dollars -> pounds)")
    print("=" * 74)

    print("\n  Gate 6: both sides vs an independent brute force")
    gate6_brute_force("A")
    gate6_brute_force("B")
    print("  gate 6 PASSED: side A anchors the enumeration, side B the reflection")

    specB = TraderSpec(L=500_000.0, T=396_000.0, A=0.00, B=0.20, side="B")
    _bi, _bt = currency_symbols(specB.side)
    print(f"\n  card: L={_bi} {specB.L:,.0f}, T={_bt} {specB.T:,.0f}, "
          f"A={specB.A:.0%}, B={specB.B:.0%}, {specB.rounds} rounds, K={specB.K}")
    t0 = time.time()
    solB = solve_v2(specB, print_progress=True)
    print(f"  solved in {time.time() - t0:.0f}s")
    print(f"  E[final wealth] = {_bi} {solB.value:,.1f}   "
          f"expected P/L = {solB.pl() * 100:+.3f}%")

    print("\n  Gates 3, 4 and 5 for the buyer")
    resB = gate3_and_4(solB, n_paths=20_000, seed=1)
    gate5_brackets(solB, resB)
    print("  gates 3, 4 and 5 PASSED for the buyer")