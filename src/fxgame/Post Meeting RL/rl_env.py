...
from fx_mechanics import GameParams, mm_accepts, trade_rate, bdc_payoff_per_unit, A0_DEFAULT
from v2_multiple_rounds import TraderSpec, terminal_wealth

P_MIN, P_MAX = 0.0, 8.0 * A0_DEFAULT      

@dataclass
class GameSpec(TraderSpec):
    L: float = 500_000.0          # starting capital, INITIAL currency
    T: float = 396_000.0          # target, TARGET currency
    A: float = 0.00               # residue penalty on leftover initial currency
...
def gate_G3(side="A", rounds=4, n_games=20_000, seed=7, print_progress=True):
    ...
    from v2_multiple_rounds import (solve_v2, greedy_offer_action,
                                    greedy_bdc_action)
    ...
    from v2_multiple_rounds import NO_FLOOR, _rate_at
