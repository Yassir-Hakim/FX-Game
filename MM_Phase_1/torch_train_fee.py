...
"""
  torch_train_fee.py -- torch_train.py with ONE change to the game: the HARDCODED MM
  accepts an offer only when the fill still profits it AFTER ITS OWN Bureau
  cost. Break-even (1-MM_FEE)*X for a pound seller, X/(1-MM_FEE) for a
  buyer, instead of X. The trader pays nothing to the MM and is never shown
  MM_FEE: it enters the verdict in rollout and nowhere else -- not a
  feature, not a normaliser, not a setting the net can read. BDC_FEE is the
  trader's OWN Bureau fee, already printed on its card; it only relocates
  here so both costs sit in one SETTINGS block.
...
from NN_one_offer_game.rl_env import Game, GameSpec, P_MIN, P_MAX
...
MM_FEE = 0.003         # the MM's OWN Bureau cost g. The trader never pays it
                      ...
                      # the naive rule and reproduces torch_train exactly
BDC_FEE = 0.02        # the trader's own Bureau fee, from its card. 0.02 is
                      # the card as printed; a knob here only so the two
                      # costs of trading sit side by side
SIDES = ("A", "B")    # every side trained in one run, each into its own folder

CARDS = {"A": dict(L=100_000.0, T=125_000.0, A=0.02, B=0.03),
         "B": dict(L=200_000.0, T=155_000.0, A=0.03, B=0.02)}
ROUNDS = 4         # None: the card's own value (4). An int overrides it on
                      # BOTH cards, so a run at another horizon is one edit.
                      # It changes the game, so it lands in its own folder
                      # (the name already carries R{rounds}) and the DP in
                      # rl_diagnostics is re-solved at that horizon
...
CHECKS = False        # check 1: this file's game must equal the certified env
...
def rollout(net, spec, rate_moves_sd, tau=None, forced=None, tau_abs=False):
    ...
    for round in range(n_rounds):
        ...
        # the MM's break-even: past its own Bureau cost, not the raw rate.
        # MM_FEE = 0 makes this hidden_rate_now exactly (multiplying or
        # dividing by 1.0 is bit-exact), which is torch_train's game
        break_even = ((1.0 - MM_FEE) * hidden_rate_now if side == "A"
                      else hidden_rate_now / (1.0 - MM_FEE))
...
def check_game_matches_fee_rule(spec, n_games=2000, seed=3):
    """check 1F -- at MM_FEE > 0 the certified env cannot be the target (its
    MM has no fee), so the target is mm_train's rule replay: an untrained
    net's HARD-game P/L here must equal mm_train.rollout under the
    fee-aware+flatten rule at this fee, to machine precision. The two files
    implement the same game independently; only the verdict centre is
    shared knowledge."""
    import mm_train  # deferred: mm_train imports train_traders imports this file
    net = build_net(SEED_INIT)
    generator = torch.Generator().manual_seed(seed)
    rate_moves_sd = torch.randn(n_games, spec.rounds + 1, generator=generator)
    rule = next(r for r in mm_train.reference_rules(MM_FEE)
                if r.name == ("fee-aware+flatten" if MM_FEE > 0
                              else "naive+flatten"))
    with torch.no_grad():
        pl_here = rollout(net, spec, rate_moves_sd, tau=None)
        pl_rule = mm_train.rollout(None, net, spec, rate_moves_sd,
                                   rule=rule, fee=MM_FEE)[1]
    gap = float((pl_here - pl_rule).abs().max())
    assert gap < 1e-12, f"check 1F FAILED: max |gap| {gap:.2e}"
    print(f"  check 1F torch game vs mm_train {rule.name} rule at fee "
          f"{MM_FEE:g}, {n_games} games (side {spec.side}): max |P/L gap| "
          f"{gap:.2e}  [ok]", flush=True)
...
# ---------------------------------------------------------------------------
def build_spec(side):
    """CARDS[side] as a GameSpec, with this file's BDC_FEE and, if set,
    ROUNDS. Every file in the study builds its cards through here."""
    spec = GameSpec(side=side, params=GameParams(bdc_fee=BDC_FEE),
                    **CARDS[side])
    if ROUNDS is not None:
        from dataclasses import replace
        spec = replace(spec, rounds=ROUNDS)
    return spec


def train_one(spec):
    # ---- 1. WHICH GAME -----------------------------------------------------
    # The card is the study's, from train_traders.CARD; this file sets only
    # its own two fees and, optionally, the horizon.
    run_directory = results_path(
        f"mm_phase1/torch_{spec.side}_R{spec.rounds}_{GATE}_g{MM_FEE:g}")
    run_directory.mkdir(parents=True, exist_ok=True)

    print(f"\n################ side {spec.side}: card L={spec.L:,.0f} "
          f"T={spec.T:,.0f} A={spec.A} B={spec.B} rounds={spec.rounds} "
          f"K={spec.K}  MM fee {MM_FEE:g}  BdC fee {BDC_FEE:g} "
          f"################", flush=True)
    ...
    if CHECKS:
        if MM_FEE == 0.0:
            check_torch_game_matches_env(spec)   # the env IS this game at g=0
            check_policy_replay_matches_env(spec)
        else:
            print(f"  checks 1-2 (vs rl_env) skipped: the certified env has "
                  f"no MM fee, so at MM_FEE={MM_FEE:g} it is a different "
                  f"game by construction. Check 1F replaces them.")
            check_game_matches_fee_rule(spec)
    ...
    # the curve, saved either way, in train_traders.py's format
    np.savez(run_directory / "curve.npz", iters=np.array(learning_curve[0]),
             hard_pl=np.array(learning_curve[1]))
    ...
    # ONLY at MM_FEE = 0. That env's MM accepts at the true rate and the v2 DP
    # is solved against the same rule, so at MM_FEE > 0 every number report()
    # produces -- the DP comparison, the replayed P/L, both path figures --
    # describes a DIFFERENT game from the one just trained. Printing them
    # would invite exactly the comparison they cannot support.
    if MM_FEE == 0.0:
        D.report(spec, torch_policy(trained_net, spec), run_directory,
                 label="torch", curve=learning_curve)
    else:
        # the fee game's own report: same shape, replayed through THIS game,
        # with the always-Bureau floor where the DP line would be
        print(f"  rl_diagnostics.report() cannot be used at MM_FEE={MM_FEE:g} "
              f"(its env and its DP both accept at the true rate); reporting "
              f"the fee game instead", flush=True)
        from trader_diagnostics import report_fee
        report_fee(spec, trained_net, learning_curve, run_directory, MM_FEE)
    print(f"artefacts in {run_directory}/  "
          f"({(time.time() - start_time) / 60:.2f} min)")
    return float(eval_pl.mean()), 2 * float(eval_pl.std()) / math.sqrt(EVAL_PATHS)


def main():
    results = {side: train_one(build_spec(side)) for side in SIDES}
    print("\n" + "=" * 72)
    print(f"BOTH SIDES, MM fee {MM_FEE:g}, BdC fee {BDC_FEE:g}, rounds "
          f"{ROUNDS if ROUNDS is not None else 'from card'}, hard game, "
          f"{EVAL_PATHS:,} paths per side")
    for side, (mean, two_se) in results.items():
        print(f"  side {side}: trader P/L {mean * 100:+.4f}% +/- "
              f"{two_se * 100:.4f}%")
